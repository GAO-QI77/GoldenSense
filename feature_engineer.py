...
class FeatureEngineer:
    ...
    def load_selected_features(self, path="selected_features.json"):
        import json
    ...
    def prepare_inference_data(self, raw_data, daily_signals=None, n_rows=60):
        """
        Prepare the latest data points for inference.
        Returns features for the last n_rows timestamps.
        """
        # 1. Preprocess
        ...
        # Ensure news columns exist even if no signal or empty signal
        for col in ['total', 'inflation', 'rates', 'risk', 'fx']:
            if f'News_{col.capitalize()}' not in df.columns:
                df[f'News_{col.capitalize()}'] = 0

        # 4. Market Features
        df = self.construct_market_features(df)
        
        # 5. Adaptive Normalize
        df = self.adaptive_normalize(df)
        
        # 6. Select Features
        # For inference, we assume the model handles the input dimension or we use all generated features 
