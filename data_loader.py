...
class MarketDataLoader:
    ...
    def fetch_data(self, period='2y', interval='1d'):
        """
        Fetches historical data for all tickers with retry logic.
        """
        data_frames = {}
        max_retries = 3
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if not df.empty:
                        ...
                        print(f"Warning: No data for {ticker}, attempt {attempt+1}")
...
if __name__ == "__main__":
    ...
    scored_news = news_loader.analyze_causality(news)
    ...
    if scored_news:
        ...
        for i in range(min(3, len(scored_news))):
            print(f"- {scored_news[i]['title']} (Score: {scored_news[i]['total']})")
            
        daily_sentiment = news_loader.get_daily_signals(scored_news)
